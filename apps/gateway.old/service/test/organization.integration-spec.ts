import { INestApplication } from '@nestjs/common';
import supertest from 'supertest';
import { getAppInstance } from './mocks/app.mock';
import { getOrganizations } from './entity/organization.mock';
import { checkProperties } from './helpers/lexicon';
import { clearDatabase } from './helpers';
import config from '../src/config/typeorm';
import { getConnection } from '../src/common/helpers';
import { DataSource } from 'typeorm';

describe('OrganisationController (integration)', () => {
  let app: INestApplication;
  let api: supertest.SuperTest<supertest.Test>;
  let connection: DataSource;

  beforeAll(async () => {
    app = await getAppInstance();
    api = supertest(app.getHttpServer());
    connection = await getConnection(config);

    await clearDatabase();
  });

  afterAll(async () => {
    await connection.destroy();

    app.close();
  });

  describe('/organisation (GET)', () => {
    it('Should find organizations, return status 200 and valid body', async () => {
      const organizations = await getOrganizations();

      return api
        .get('/organisation')
        .send()
        .expect(200)
        .then((res) => {
          expect(Array.isArray(res.body)).toBeTruthy();
          expect(res.body.length).toEqual(organizations.length);

          for (const key in organizations) {
            checkProperties(res.body[key], organizations[key], ['id', 'name']);
          }
        });
    });
  });
});
