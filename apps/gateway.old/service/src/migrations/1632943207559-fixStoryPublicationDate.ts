...
export class FixStoryPublicationDate1632943207559
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const item of items) {
      operations.push(
        queryRunner.query(
          `UPDATE \`${this.tableName}\` SET \`updated_at\` = \`published_at\`, \`published_at\` = ? WHERE id = ?`,
          [item.published_at, item.story_id],
        ),
      );
    }
