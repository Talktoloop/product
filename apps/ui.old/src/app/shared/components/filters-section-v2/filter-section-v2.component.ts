...
export class FilterSectionV2Component extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.isInboxOrOutbox = ['inbox', 'outbox'].includes(this.router.url.split('/')[1]);
  }
  ...
  get visibleConfig(): IFilterV2<any>[] {
    return this.config?.filter((singleConfig) => this.isAuthenticated && !singleConfig.internalName.includes('SearchText')) || [];
  }
