...
import { REPLIED_TO_MAPPING, REPLIED_TO_VALUE } from '@shared/types/replied-to.type';
...
export class MetaDataService extends ApiService {
  ...
  getRepliedTo(): Observable<IBaseEntityCheck[]> {
    const items = of(
      Object.values(REPLIED_TO_VALUE).map((repliedToId) => {
        return { id: repliedToId.toString(), value: repliedToId.toString(), checked: false, code: REPLIED_TO_MAPPING[repliedToId] };
      }),
    ).pipe(
      tap((repliedTo) => {
        this.repliedTo$.next(repliedTo);
      }),
    );
    return items;
  }
