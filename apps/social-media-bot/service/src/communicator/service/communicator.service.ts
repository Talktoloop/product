...
export class CommunicatorService {
  ...
  async prepareChatMessage(
    ...
    const introKeyTTL = this.configService.get('application').ttl.moderatorFlow;
    
    const hasIntroBeenSent = await this.redisService.getModeratorIntroFlag(
      data.senderId,
      data.pageId,
    );
    ...
    if (!hasIntroBeenSent) {
      ...
      await this.redisService.setModeratorIntroFlag(data.senderId, data.pageId, introKeyTTL);
    }
  ...
  getLastQuickReplyMessage(data): string {
