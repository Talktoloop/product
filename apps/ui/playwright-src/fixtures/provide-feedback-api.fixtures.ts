...
export class ProvideFeedbackFixture extends BaseAPI {
  ...
  async createFeedbackViaApi(content = this.feedbackContent.body): Promise<FeedbackData> {
  ...
  async createAndEditFeedback(content?: string): Promise<CreateAndEditFeedback> {
    const feedback = await this.createFeedbackViaApi(content);
    const storyID = await this.getFeedbackID();
    await this.updateFeedbackBeforePublish(storyID);
  ...
  async openFeedback(page: Page, feedbackId: CreateAndEditFeedback): Promise<ReviewFeedbackView> {
    await page.goto(`inbox/stories/story/web/${feedbackId.storyID}/review`);
    return new ReviewFeedbackView(page);
  }
}
