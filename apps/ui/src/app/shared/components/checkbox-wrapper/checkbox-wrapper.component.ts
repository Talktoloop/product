...
export class CheckboxWrapperComponent extends ControlValueAccessorBase<number[] | number> {
  ...
  private prepareChecboxValuesAndEmitChange(): void {
    ...
    const selectedChangeValue = Array.isArray(filteredCheckboxes) ?
      filteredCheckboxes.length : filteredCheckboxes >= 0 ? 1 : 0;

    const onChangeValue = Array.isArray(filteredCheckboxes) && filteredCheckboxes.length > 0 ?
      filteredCheckboxes : null;
