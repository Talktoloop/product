...
export class FiltersService {
  ...
  get filtersData$(): Observable<IFiltersData> {
    return combineLatest([
      ...
      this.metaDataService.repliedTo$,
      this.metaDataService.channelFilter,
    ]).pipe(
      map(([categories, difficulties, organisations, thematic, ages, genders, minority, repliedTo, channelFilter]) => {
        return {
          countries: this.countriesService.countriesModel(),
  ...
  constructor(private metaDataService: MetaDataService, private countriesService: CountriesService) {}
