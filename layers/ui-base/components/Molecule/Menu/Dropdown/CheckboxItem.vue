...
import type { MenuItemCheckbox } from '@ui/base/types'
...
<template>
  <shadcn-dropdown-menu-checkbox-item v-bind="$attrs" :checked="item.checked" :disabled="item.disabled"
    @update:checked="emit('update:checked', $event)">
    <component :is="item.icon" v-if="item.icon" class="mr-2 h-4 w-4" />
    <span :class="{ 'ml-6': item.inset }">{{ item.label }}</span>
